...
if (2.1 in persondex["Instructor Bertha"]["ClassesKnown"]):
    menu:
        "{b}>General Studies{/b}":
            pass
        ...
        ">Craft items" if (3.1 in persondex["Instructor Bertha"]["ClassesKnown"]):
            jump itemcraftground

if (persondex["Instructor Bertha"]["ClassesKnown"] == []): #first class
    $ persondex["Instructor Bertha"]["ClassesKnown"].append(1)
    $ renpy.pause(1.0, hard=True)
    ...
    ethan thinking "Then again, nobody told me anything about this school in general..."
    ...
    brendan talkingmouth "Are you okay?{w=0.5} You're not bleedin' or anything are you?"
    
    red happy "I'm fine. It's only a pencil. Still, watch where you fling those things!"

    brendan thinking "Shoot, my bad, man.{w=0.5} It's kind of a bad habit of mine. Nervous fingers, y'know?"
    brendan -thinking "When I get bored, I start spinnin' pencils. You know, like those guys do in the movies."
    ...
    red -happy "Well, keep practicing, then. We'll need your deadly skills when Hilbert acts up."
    ...
    hide bertha
elif (2.1 not in persondex["Instructor Bertha"]["ClassesKnown"] and classstats["Ground"] >= 10):#Burial Ground
    show bertha with dis
    if (2 not in persondex["Instructor Bertha"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        ...
        bertha @norm4 "You look concerned, youngster."

        red uniform @thinking "Maybe less concerned, and more confused."
        ...
        red @thinking "I did wonder why the camera went shaky so often."
        ...
        red @thinking "Hmm... so this movie {i}did{/i} actually happen..." 
        red @thinking "Those two Pokémon are legends. People didn't even think they were real, right? I mean, I know {i}a lot{/i} about Pokémon, and I'd only heard of them as myths." 
        ...
        bertha sad @sad2 "I... had a sister that grew up there."
        ...
        bertha norm @happy2 "Oh, never mind that, youngster. I ask if you came from Pallet Town, because, knowing that, it makes sense to me that you would not be aware of the world at large."

        red @sad "Oh... so I've got a lot of catching up to do, huh?"

        bertha @norm2 "Perhaps. I'm not sure that's necessary, though."

        red @thinking "What do you mean, Instructor?"
        ...
        red @thinking "What lesson are we supposed to be getting from this movie? Is it that Ground-types are responsible for the world's formation?"
        ...
        bertha @happy2 "But keep that one to yourself, alright, youngster? Besides, you've all been working so hard, you deserve a break. Popcorn?"

        red @thinking "Oh, no thanks. I just... Hm."
        ...
        red @thinking "Huh... okay, I can see the implications there."
        ...
        $ persondex["Instructor Bertha"]["ClassesKnown"].append(2)
    else:
    ...
    bertha @norm2 "Ready, youngster? I won't be holding back!"
    $ hidebattleui=False
    $ mustswitch = False
    ...
    $ gymbattles["Instructor Bertha1"]  = _return

    show bertha with dis

    if (WonBattle("Instructor Bertha1")):
        $ persondex["Instructor Bertha"]["ClassesKnown"].append(2.1)

        bertha @happy2 "Good job, youngster! Victory treacle?"
        ...
        bertha @sad2 "Oh, dearie me... Perhaps I went too hard, then?"
...
elif (3.1 not in persondex["Instructor Bertha"]["ClassesKnown"] and classstats["Ground"] >= 20):#Soft Sand
    show bertha with dis
    if (3 not in persondex["Instructor Bertha"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        
        $ lowertime = timeOfDay.lower()

        bertha @norm2 "Good [lowertime], youngster."
        ...
        bertha @happy2 "Oh, it's one of my few pleasures left. Old duffers like myself don't have anything left to do but worry about those younger than us."
        
        red @happy "Now, I {i}know{/i} that's not true, Instructor. You're a brilliant teacher."

        bertha @norm2 "Well, you're very kind for saying that. But, really, what is teaching except worrying about the youth?"

        red @thinking "Hm. Not sure I know that one."
        ...
        $ persondex["Instructor Bertha"]["ClassesKnown"].append(3)
    else:
    ...
    $ gymbattles["Instructor Bertha2"]  = _return

    show bertha with dis

    if (WonBattle("Instructor Bertha2")):
        $ persondex["Instructor Bertha"]["ClassesKnown"].append(3.1)

        bertha @happy2 "Good job, youngster! Here's a packet of Soft Sand. I made this ahead of time, since I figured you were going to be taking the exam any day now."

        $ GetItem("Soft Sand", 1, "You gained a Soft Sand! The pouch appears to be hand-embroided, bearing your name.")
    ...
    hide bertha with dis
elif (4.1 not in persondex["Instructor Bertha"]["ClassesKnown"] and classstats["Ground"] >= 30):#Bulldoze
    show bertha with dis
    if (4 not in persondex["Instructor Bertha"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        ...
        $ persondex["Instructor Bertha"]["ClassesKnown"].append(4)
    else:
        red uniform @talking2mouth "Instructor Bertha, I've trained and studied since our last battle, and I think I can do it this time."
    ...
    $ gymbattles["Instructor Bertha3"]  = _return

    show bertha with dis

    if (WonBattle("Instructor Bertha3")):
        $ persondex["Instructor Bertha"]["ClassesKnown"].append(4.1)
...
show bertha with dis
bertha @norm2 "Oh, you'd like to teach your Ground-type a new move? Certainly, youngster. I can do that in a jiffy."
bertha @norm2 "I can teach Burial Ground, which is a move my sister and I made that slows and damages all grounded non-Ground and non-Ground types on the field for five turns."
if (4.1 in persondex["Instructor Bertha"]["ClassesKnown"]):
...
menu:
    ...
    ">Learn Bulldoze" if (4.1 in persondex["Instructor Bertha"]["ClassesKnown"]):
        $ taughtmove = "Bulldoze"
    "Nevermind":
...
menu:    
    ">Craft Soft Sand" if (3.1 in persondex["Instructor Bertha"]["ClassesKnown"]):
