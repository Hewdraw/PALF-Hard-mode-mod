...
if (2.1 in persondex["Sensei Marshal"]["ClassesKnown"]):
    menu:
        "{b}>General Studies{/b}":
            pass
        ...
        ">Craft items" if (3.1 in persondex["Sensei Marshal"]["ClassesKnown"]):
            jump itemcraftfighting

if (persondex["Sensei Marshal"]["ClassesKnown"] == []): #first class
    $ persondex["Sensei Marshal"]["ClassesKnown"].append(1)
    $ renpy.pause(1.0, hard=True)
    ...
    ethan -happy "Sounds like a plan."
    ...
    play sound "Audio/Marshal Punches.ogg"
    ...
    marshal -surprised "Welcome to my {color=#0048ff}Fighting dojo!{/color} I am your sensei--"
    ...
    play sound "Audio/Wood Break.ogg"
    ...
    ethan @thinking "I don't know anything about martial arts, but I guess that was impressive?"

    play sound "Audio/Scattered Applause.ogg"

    red @thinking "The uneasy applause coming from the class is making me cringe, but he seems to be taking it in stride."
    ...
    if (calDate.month == 4 and calDate.day == 5):
        ...
        calem thinking "Uh... I'm not too familiar with martial arts, but isn't this an academic class?"
        calem -thinking "I thought we'd be here learning material about Pokémon from textbooks and whatnot."
    ...
    "Whole Class" "\"{size=26}{i}{cps=15}N-{w=0.25}Nadeshiko{/cps}.{w=0.25}.{w=0.25}.{/i}{/size}\""

    marshal "{size=44}Too quiet! {i}Mo ichi do!{/i} {/size}"
    marshal "{size=44}{i}YAMATO!{/i} {/size}"

    "Whole Class" "\"{i}Nadeshiko!{/i}\""
    ...
    hide marshal
elif (2.1 not in persondex["Sensei Marshal"]["ClassesKnown"] and classstats["Fighting"] >= 10):#Disabling Poke
    show marshal with dis
    if (2 not in persondex["Sensei Marshal"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        ...
        red @thinking "Um... use three other moves?"
        ...
        marshal @think2 "Indeed, I do not. But a subtle, quick, move might get underneath your opponent's guard in a way that they cannot react to."
        ...
        $ persondex["Sensei Marshal"]["ClassesKnown"].append(2)
    else:
    ...
    $ gymbattles["Sensei Marshal1"]  = _return

    show marshal with dis

    if (WonBattle("Sensei Marshal1")):
        $ persondex["Sensei Marshal"]["ClassesKnown"].append(2.1)

        marshal @think2 "{i}Arigatou.{/i} That was a well-fought battle."
...
elif (3.1 not in persondex["Sensei Marshal"]["ClassesKnown"] and classstats["Fighting"] >= 20):#Black Belt
    show marshal with dis
    if (3 not in persondex["Sensei Marshal"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        ...
        red @thinking "No, not really. I use a cream for that."
        ...
        red @thinking "Hm... well, it shows everyone around you what rank of martial artist you are? So you don't have to tell them?"
        ...
        red @thinking "...Hm. So... if I give a Pokémon a Black Belt, in recognition of some achievement, which doesn't necessarily have to be the full process of getting a Black Belt a human would go through... it would help them?"
        ...
        $ persondex["Sensei Marshal"]["ClassesKnown"].append(3)
    else:
        red uniform @talking2mouth "Sensei Marshal, I have trained diligently since our last meeting--and now I am ready to reclaim my honor!"
    ...
    $ gymbattles["Sensei Marshal2"]  = _return

    show marshal with dis

    if (WonBattle("Sensei Marshal2")):
        $ persondex["Sensei Marshal"]["ClassesKnown"].append(3.1)
    ...
    hide marshal with dis
elif (4.1 not in persondex["Sensei Marshal"]["ClassesKnown"] and classstats["Fighting"] >= 30):#Force Palm
    show marshal with dis
    if (4 not in persondex["Sensei Marshal"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        ...
        red @thinking "Hm... well, early on in your classes, I think I would have assumed you'd want us to make a fist. But then you taught me Disabling Poke."
        red @thinking "But you made clear Disabling Poke is a specialty move, meant to be used in specific situations. So..."
        ...
        $ persondex["Sensei Marshal"]["ClassesKnown"].append(4)
    else:
    ...
    $ gymbattles["Sensei Marshal3"]  = _return

    show marshal with dis

    if (WonBattle("Sensei Marshal3")):
        $ persondex["Sensei Marshal"]["ClassesKnown"].append(4.1)
...
if (4.1 in persondex["Sensei Marshal"]["ClassesKnown"]):
...
menu:
    ...
    ">Learn Force Palm" if (4.1 in persondex["Sensei Marshal"]["ClassesKnown"]):
...
menu:    
    ">Craft Black Belt" if (3.1 in persondex["Sensei Marshal"]["ClassesKnown"]):
