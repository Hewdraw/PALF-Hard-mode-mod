...
if (2.1 in persondex["Lieutenant Surge"]["ClassesKnown"]):
    menu:
        "{b}>General Studies{/b}":
            pass
        ...
        ">Craft items" if (3.1 in persondex["Lieutenant Surge"]["ClassesKnown"]):
            jump itemcraftelectric

if (persondex["Lieutenant Surge"]["ClassesKnown"] == []): #first class
    $ persondex["Lieutenant Surge"]["ClassesKnown"].append(1)
    $ renpy.pause(1.0, hard=True)
    ...
    red uniform @thinking "...I'm not going to 'get in there.' I'm just going to talk."
    ethan "Sure, sure."

    pause 1.0

    leaf flirt "Hey, another class together!{w=0.5} You must feel so happy to spend {i}so much time{/i} with such a beautiful girl!"
    red "I'm kind of feeling something different."
    leaf @talkingmouth "Oh, yeah? Let me guess, is it blessed?{w=0.5} Lucky?"
    ...
    surge "You recruits better start gettin' your acts together, unless you {i}WANT{/i} to feel my combat boots up your adolescent assholes!"

    "Spineless Student" "H-hey! Y-y-y-you can't treat us l-like this!"
    ...
    "Spineless Student" "Y-you're a teacher here, right?{w=0.5} Teachers c-can't treat s-students like this!"
    "Spineless Student" "That's against school p-p-policy...{w=0.5} isn't it?"
    ...
    surge angrybrow anger @angrymouth "Do you think I'm cute, boy?!{w=0.5} Do you think I'm playing JOKES?!"

    "Spineless Student" "{cps=18}W-Wha--{/cps}"

    surge @angrymouth "I've been given special permission to teach brats like YOU the meaning of discipline and respect!"
    surge -anger biganger @angrymouth "If you can't handle the training, then crawl back home and suck on your mama's tits!"
    ...
    surge @angry "Now, OUT! I won't have goddamn crying babies in here!"
    ...
    surge -surprised angry biganger "{cps=*0.2}WHO LAUGHED?"
    ...
    nate thinking "I thought that was obvious, actually."
    ...
    surge "Nate, I oughta choke the life out of you right here, right now!"
    ...
    extend " Let's go! I've seen legless Shuckles move faster than you assbags!"

    red "Oh, god. If this is how it's gonna be every class, I might just end it all."
    ...
    rosa happybrow talkingmouth "No... I recognize an actor when I see one, and he was definitely putting a show on for the new students."

    if (not IsDate(5, 4, 2004)):
        red thinking "Geez, I didn't think he even noticed Ethan and I were new. And I was happy that way!"

    red "Well, Rosa, we should probably head out now. Don't want him yelling at us. Talk to you later?"
    rosa happy "Sure!"

    hide rosa with dis

    hide surge
elif (2.1 not in persondex["Lieutenant Surge"]["ClassesKnown"] and classstats["Electric"] >= 10):#Energize
    show surge with dis
    if (2 not in persondex["Lieutenant Surge"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        ...
        surge @talkingmouth "You've been taking these classes long enough that you assbags should be ready to learn the good stuff!"

        red @happy "Oh? Maybe something good's about to happen!"

        surge angry anger "HEY! Wipe that dumbass smile off your face, Private! Do you think you're good enough to advance in this class?"
        ...
        surge -anger -biganger -angry @happy "You're goddamn right, you will! It's banned by the international warfare commission for being too powerful!"
        surge @surprised "That move not only increases the power of your next Electric-type move, it also increases BOTH your attack stats!"
        ...
        surge -surprised @happy "...That's what I like to hear! A real man always defends his mama's cooking!"
        ...
        $ persondex["Lieutenant Surge"]["ClassesKnown"].append(2)
    else:
        red uniform @talking2mouth "Lieutenant Surge, I'm prepared to battle you again, Sir!"

    surge @happy "Pick one Pokémon to save your ass from this beating! {color=#0048ff}If you pick anything but an Electric-type, then you better go back to your pops' funny farm!{/color}"
    ...
    surge "If you can't beat me, then you haven't been paying attention in my classes! And then I'll REALLY be mad!"
    $ hidebattleui=False
    $ mustswitch = False
    ...
    $ gymbattles["Lieutenant Surge1"]  = _return

    show surge with dis

    if (WonBattle("Lieutenant Surge1")):
        $ persondex["Lieutenant Surge"]["ClassesKnown"].append(2.1)
        ...
        surge @anger "I expect more from MY students! Get out of this classroom until you've trained more!"

        redmind uniform @thinking "Damn... that was an embarrassing loss. Still, at least I learned something..."

    hide surge with dis
elif (3.1 not in persondex["Lieutenant Surge"]["ClassesKnown"] and classstats["Electric"] >= 20):#Magnet
    show surge with dis
    if (3 not in persondex["Lieutenant Surge"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        ...
        surge -angry @happy "GOOD! Because you got a 100%%! Listen up, LADIES! Private [first_name] has just volunteered himself as a tutor to all of you assbags who can't keep up with the program!"
        ...
        $ persondex["Lieutenant Surge"]["ClassesKnown"].append(3)
    else:
    ...
    $ gymbattles["Lieutenant Surge2"]  = _return

    show surge with dis

    if (WonBattle("Lieutenant Surge2")):
        $ persondex["Lieutenant Surge"]["ClassesKnown"].append(3.1)
    ...
    hide surge with dis
elif (4.1 not in persondex["Lieutenant Surge"]["ClassesKnown"] and classstats["Electric"] >= 30):#Shock Wave
    show surge with dis
    if (4 not in persondex["Lieutenant Surge"]["ClassesKnown"]):
        $ renpy.pause(1.0, hard=True)
        ...
        $ persondex["Lieutenant Surge"]["ClassesKnown"].append(4)
    else:
    ...
    $ gymbattles["Lieutenant Surge3"]  = _return

    show surge with dis

    if (WonBattle("Lieutenant Surge3")):
        $ persondex["Lieutenant Surge"]["ClassesKnown"].append(4.1)
...
if (4.1 in persondex["Lieutenant Surge"]["ClassesKnown"]):
...
menu:
    ">Learn Energize":
        $ taughtmove = "Energize"
    ">Learn Shock Wave" if (4.1 in persondex["Lieutenant Surge"]["ClassesKnown"]):
...
menu:    
    ">Obtain Magnet" if (3.1 in persondex["Lieutenant Surge"]["ClassesKnown"]):
