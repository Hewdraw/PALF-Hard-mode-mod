...
if (topval > aimlevel):
    ...
    red @thinking "Right. And, Professor, what would you recommend my levels be around?"
    ...
    red @thinking "Hm... that sounds like Gardenia..."
...
python:
    ...
    skyladucklettobj.UpdateMoves(["Tailwind", "Water Pulse", "Wing Attack"])
    skyladucklettobj.Intelligence = 1
    skyladucklettobj.EVs = [1, 0, 1, 0, 1, 0]
    skylalaprasobj.UpdateMoves(["Healing Spring", "Icy Wind", "Water Gun", "Ice Shard"])
    skylalaprasobj.Intelligence = 1
    skylalaprasobj.EVs = [1, 0, 1, 0, 1, 0]
    skylaemolgaobj = Pokemon("Emolga", level=13, moves=[GetMove("Spark"), GetMove("Nuzzle"), GetMove("Electroweb"), GetMove("Quick Attack")], ivs=[24, 24, 24, 24, 24, 24], evs=[1, 0, 1, 0, 1, 0], gender=Genders.Female, nature=Natures.Jolly, ability="Motor Drive", intelligence=1)
    skylaskarmoryobj = Pokemon("Skarmory", level=13, moves=[GetMove("Metal Claw"), GetMove("Peck"), GetMove("Roost")], ivs=[24, 24, 24, 24, 24, 24], evs=[1, 0, 1, 0, 1, 0], gender=Genders.Female, nature=Natures.Bold, ability="Keen Eye",intelligence=1)
    skylaparty = [skyladucklettobj, skylalaprasobj, skylaemolgaobj, skylaskarmoryobj]

    if (skylahasmantyke):
        skylamantykeobj = Pokemon("Mantine", level=8, moves=[GetMove("Bubble Beam"), GetMove("Roost"), GetMove("Bullet Seed"), GetMove("Psybeam")], ivs=[24, 24, 24, 24, 24, 24], gender=Genders.Female, nature=Natures.Jolly, ability="Swift Swim")
        #skylaremoraidobj = Pokemon("Remoraid", level=13, moves=[GetMove("Psybeam"), GetMove("Water Gun"), GetMove("Protect"), GetMove("Rain Dance")], gender=Genders.Female, nature=Natures.Serious, ability="Moody")
        skylaparty.append(skylamantykeobj)

    trainer2 = Trainer("skyla", TrainerType.Enemy, skylaparty, number=2)

call Battle([trainer1, trainer2], uniforms=[True, True]) from _call_Battle_69
$ gymbattles["Skyla1"]  = _return
