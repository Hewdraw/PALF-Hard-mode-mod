...
alder "...And that's why it's important to memorize your type matchups! At the very least, memorize what the Rock-type is super-effective against."
...
alder "Anyway, that's the end of this lecture. Just remember to pack a Rapid Spinner!"
...
alder -surprised @happy2 "Ah, there I go again. I guess I tend to overstate how much Stealth Rocks matter."
...
python:
    tiamimejrobj = Pokemon(pokedexlookupname("Mime Jr.", DexMacros.Id), moves=[GetMove("Confusion"), GetMove("Round")], level=9, ivs=[24, 24, 24, 24, 24, 24], nature=Natures.Hardy, gender=Genders.Male, ability="Technician", intelligence=1)
    trainer1 = Trainer("red", TrainerType.Player, playerparty, number=2)
    trainer2 = Trainer("tia", TrainerType.Enemy, [
        tiamimejrobj,
        Pokemon(pokedexlookupname("Noibat", DexMacros.Id), moves=[GetMove("Air Cutter"), GetMove("Legacy")], level=10, ivs=[24, 24, 24, 24, 24, 24], nature=Natures.Modest, gender=Genders.Male, ability="Infiltrator", intelligence=1),
        Pokemon(pokedexlookupname("Latias", DexMacros.Id), moves=[GetMove("Dragon Breath"), GetMove("Confusion"), GetMove("Recover"), GetMove("Refresh")], level=13, ivs=[31, 31, 31, 31, 31, 31], nickname="Tia", nature=Natures.Modest, gender=Genders.Female, ability="Levitate", intelligence=1)
    ])
    trainer3 = Trainer("bianca", TrainerType.Enemy, [
        Pokemon(pokedexlookupname("Munna", DexMacros.Id), moves=[GetMove("Psybeam"), GetMove("Hypnosis"), GetMove("Defense Curl"), GetMove("Yawn")], level=11, ivs=[24, 24, 24, 24, 24, 0], nickname="Moony", nature=Natures.Quiet, gender=Genders.Female, ability="Synchronize"),
        Pokemon(pokedexlookupname("Minccino", DexMacros.Id), moves=[GetMove("Echoed Voice"), GetMove("Helping Hand"), GetMove("Baby-Doll Eyes"), GetMove("Pound")], level=9, ivs=[24, 24, 24, 24, 24, 24], nickname="Minnie", nature=Natures.Jolly, gender=Genders.Female, ability="Technician")
    ])

call Battle([trainer1, trainer2, trainer3], uniforms=[True, True, True]) from _call_Battle_48
$ gymbattles["BiancaTia1"]  = _return
