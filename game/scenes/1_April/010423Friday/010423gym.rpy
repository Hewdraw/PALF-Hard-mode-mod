...
if (GetRelationship("May") == "Taster"):
    ...
    if (WonBattle("BrendanMay1")):
        ...
        redmind @confused "No, seriously, who {i}says{/i} that?"
...
python:
    maytorchicobj.Level = 13
    maytorchicobj.RecalculateStats()
    maytorchicobj.Moves = [GetMove("Flame Charge"), GetMove("Steady Flame"), GetMove("Quick Attack")]
    maytorchicobj.Intelligence = 1
    maytorchicobj.EVs = [0, 1, 0, 1, 0, 1]
    mayvenonatobj.Moves = [GetMove("U-turn")]
    mayvenonatobj.EVs = [0, 1, 0, 1, 0, 1]
    mayvenonatobj.Intelligence = 1
    mayheracrossobj = Pokemon(pokedexlookupname("Heracross", DexMacros.Id), level=12, gender=Genders.Male, moves=[GetMove("Pin Missile"), GetMove("Arm Thrust"), GetMove("Bullet Seed"), GetMove("Aerial Ace")], ivs=[24, 24, 24, 24, 24, 24], evs=[0, 1, 0, 1, 0, 1], nature=Natures.Adamant, ability="Guts", intelligence=1)
    

    trainer1 = Trainer("red", TrainerType.Player, playerparty, number=2)
    trainer2 = Trainer("may", TrainerType.Enemy, [maytorchicobj, mayvenonatobj, mayheracrossobj], number=2)

call Battle([trainer1, trainer2], uniforms=[True, True]) from _call_Battle_51
$ gymbattles["May1"]  = _return
...
may @thinking "So where could I get a Larvesta egg?"
