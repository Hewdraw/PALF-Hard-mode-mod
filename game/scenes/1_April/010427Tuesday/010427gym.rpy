...
if (GetRelationship("Bea") == "Planner"):
    ...
    bea @thinking "But what you {i}have{/i} done means a lot to me."
    ...
    bea @closedbrow "They're far too complicated. After a while, each of my plans that related to defeating you now required me to have set events in motion several weeks before our battle."
...
$ beaclobbopusobj = Pokemon("Clobbopus", level=12, gender = Genders.Male, moves=[GetMove("Disabling Poke")], ivs=[24, 24, 24, 24, 24, 24], nature=Natures.Adamant, ability="Technician")
$ bearolycolyobj = Pokemon("Rolycoly", level=12, gender = Genders.Female, moves=[GetMove("Smack Down"), GetMove("Rapid Spin")], ivs=[24, 24, 24, 24, 24, 24], evs=[1, 1, 1, 1, 1, 1], nature=Natures.Adamant, ability="Flash Fire", intelligence=1)
$ beamachopobj = Pokemon("Machop", level=12, gender = Genders.Male, moves=[GetMove("Low Sweep")], ivs=[24, 24, 24, 24, 24, 24], nature=Natures.Adamant, ability="No Guard")
$ beatyrogueobj = Pokemon("Tyrogue", level=6, gender = Genders.Male, moves=[GetMove("Tackle"), GetMove("Fake Out"), GetMove("Helping Hand"), GetMove("Foresight")], ivs=[24, 24, 24, 24, 24, 24], nature=Natures.Adamant, ability="Steadfast")
$ beafalinksobj = Pokemon("Falinks", level=12, moves=[GetMove("Rock Smash"), GetMove("Knock Off")], nature=Natures.Adamant, ivs=[24, 24, 24, 24, 24, 24], evs=[0, 1, 0, 1, 0, 1], ability="Defiant", intelligence=1)
$ beaparty  = [beaclobbopusobj, bearolycolyobj, beamachopobj, beafalinksobj]
if (beahastyrogue):
    $ beaparty.append(beatyrogueobj)

$ HealParty()
$ trainer1 = Trainer("red", TrainerType.Player, playerparty, number=2)
$ trainer2 = Trainer("bea", TrainerType.Enemy, beaparty, number = 2)

call Battle([trainer1, trainer2], uniforms=[True, True]) from _call_Battle_65
$ gymbattles["Bea1"]  = _return
