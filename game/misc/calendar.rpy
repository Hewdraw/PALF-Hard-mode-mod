...
label calendar(direction):
    ...
    if (investment > 0 and not IsBefore(25, 4, 2004) and getRWDay(1) not in ["Saturday", "Sunday"] and not IsDate(2, 5, 2004) and not IsDate(3, 5, 2004) and not IsDate(4, 5, 2004)):
        $ gain = math.floor(investment * 0.05)
        $ money += gain
        $ gains += gain
...
screen GardeniaPayment(amount):
    if (IsDate(5, 5, 2004) or (IsDate(6, 5, 2004) and timeOfDay == "Morning")):
        text "Gardenia found your new room and dropped off $" + str("{:,}".format(amount)) + "!" size 48 color "#ffffff" xanchor 1.0 yanchor 1.0 at swipe_in(800, 1000)
    else:
        text "Gardenia dropped off $" + str("{:,}".format(amount)) + "!" size 48 color "#ffffff" xanchor 1.0 yanchor 1.0 at swipe_in(800, 1000)
