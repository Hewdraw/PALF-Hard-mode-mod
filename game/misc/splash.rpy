...
label CreateSplash(players, challengers, customexpressions=[], reanchor=[], uniforms=[], pkmnlist=[]):        
    # setup art
    hide blank2
    ...
    python:
        ...
        for i, player in enumerate(players):
            ...
            charcolor = getCharColor(charname.capitalize()) if player not in pkmnlist and player != "sideportraitfull" else GetColor(pokedexlookupname(player.capitalize(), DexMacros.Id))
            finalmatrix = TintMatrix(charcolor) * BrightnessMatrix(1.0) * ContrastMatrix(0.0)
            playersprite = (player if len(customexpressions) == 0 else customexpressions[i * 2]) + (" uniform" if uniforms[i] else "") + (" hat" if player == "tia" else "")
        ...
        for i, challenger in enumerate(challengers):
            ...
            challengername = pokedexlookupname(challenger.capitalize(), DexMacros.Id) if challenger != "sideportraitfull" else sidemonnum
            ...
            finalmatrix = TintMatrix(charcolor) * BrightnessMatrix(1.0) * ContrastMatrix(0.0)
            challengersprite = (challenger if len(customexpressions) == 0 else customexpressions[i + len(players) * 2]) + (" uniform" if uniforms[i + len(players)] else "") + (" hat" if challenger == "tia" else "")
            newtransforms = [Transform(yanchor=(1.0 if reanchor[i + len(players)] else 0.65), xanchor=0.5), fadeinright(xbuffer)] + ([Transform(ypos=0.7)] if challenger in pkmnlist or challenger == "sideportraitfull" else [])
