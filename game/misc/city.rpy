...
$ renpy.music.queue("audio/music/inspira.ogg", channel='music', loop=True, fadein=1.0, tight=None)
...
if (not trainer1.HasMons()):
    ...
    menu:
        ...
        "{b}Seek out Silver's Gang{/b}" if (lastfight + datetime.timedelta(days=3) <= calDate):
            show abandonedhouse
            show roughneck 
            with dis
            ...
            python:
                ...
                trainers.append(Trainer("red", TrainerType.Player, playerparty, numfoes))
                ...
                for i in range(numfoes):
                    ...
                    for i in range(teamnumber):
                        newteam.append(Pokemon(RandomChoice(gruntmons), level = RandomChoice(range(highestlevel, highestlevel + 2)), ivs=[24, 24, 24, 24, 24, 24]))
                    newtrainer = Trainer("roughneck", TrainerType.Enemy, newteam, 1)
            ...
            if (_return):
                ...
                $ totalwinnings = math.floor((1 + numfoes / 10.0) * (1 + numenemymons / 10.0) * 30 * highestlevel)
                $ money += totalwinnings
        ...
        "Buy Items":
            call screen shopkeep(GetShopItems())
            $ boughtitem = _return
            if (boughtitem == "Back"):
                jump citysetup
            else:
                $ totalcost = boughtitem[1] * boughtitems
                if (totalcost > money):
                    narrator "You can't afford that!"
                else:
                    $ itemname = boughtitem[0]
                    $ money -= totalcost 
                    if (boughtitems > 1):
                    ...
                    if (boughtitems >= 10 and "Ball" in itemname):
...
label cramorantscene:
    ...
    if (not seencramorant):
        red @thinking "Hm... looks like the seaport isn't super-busy right now. I don't see any signs of a Frenzied Pokémon around, either..."
        ...
        red @thinking "Of course, that Absol and Lopunny were both pretty far out of their territory. I guess there's no guarantee that whatever stole Tia's hat came from this area."
        ...
        red @thinking "Okay. We need to make sure that Cramorant doesn't get the drop on us again. With your electric-type moves, it wouldn't stand a chance. Ready, buddy?"
    ...
    $ PlaySound("pokemon/cries/845.2.wav")
    ...
    python:
        ...
        cramorantobj = Pokemon("Cramorant", level=25, moves=[GetMove("Surf"), GetMove("Icy Wind"), GetMove("Air Cutter"), GetMove("Roost")])
    ...
    $ gymbattles["Cramorant1"]  = _return

    $ AddPikachu()

    if (cramorantobj in (playerparty + box)):
        python:
            seaportunlocked = True
            cramorantobj.Level = 11
            cramorantobj.Moves = [GetMove("Stockpile"), GetMove("Swallow"), GetMove("Spit Up"), GetMove("Water Gun")]
            cramorantobj.Experience = cramorantobj.CalculateAllExperienceNeededForLevel(11)
        ...
        elif (absolobj in (playerparty + box) or lopunnyobj in (playerparty + box)):
            red @happy "I bet [pronoun2]'ll probably stop trying to eat you. These Frenzied Pokémon always calm down after a while."
        else:
            red @thinking "As long as [pronoun2] stops trying to eat you, anyway."
        ...
        red @thinking "I get the feeling that [pika_name] would rather not hang out around [cramorantname] any more than absolutely necessary..."   
...
label wallyellowscene:
    ...
    play music "audio/music/relicsong.mp3"
    ...
    yellow @thinking "Actually... I think Leaf might have mentioned one of her old roommates was named May..."
